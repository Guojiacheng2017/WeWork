...
const services = {
  ...
  setSdhConnection: async (input) => { const value=await sdhConnection.set(input); const health=await sdh.health(); if(health.service!=='smalldashharness')throw Object.assign(new Error('目标不是 smalldashharness 服务'),{code:'HARNESS_PROTOCOL_UNSUPPORTED'}); try { await sdh.models(); } catch(error) { throw Object.assign(new Error(`SDH 已响应 /health，但模型接口不可用：${error.message}。请更新远程 SDH 镜像。`),{code:'HARNESS_PROTOCOL_UNSUPPORTED'}); } return {...value,reachable:true,service:health.service}; },
