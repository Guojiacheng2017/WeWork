...
export default function weworkPiExtension(pi) {
  pi.on('tool_call', async (event) => {
    if (event.toolName.startsWith('wework_')) return;
    try {
      const response = await fetch(process.env.WEWORK_PI_TOOL_URL, {method:'POST', headers:{authorization:`Bearer ${process.env.WEWORK_PI_TOOL_TOKEN}`, 'content-type':'application/json'}, body:JSON.stringify({name:'__native_admission', arguments:{toolName:event.toolName}})});
      const result = await response.json();
      if (!response.ok) return {block:true,reason:result.error || 'Production admission denied'};
    } catch { return {block:true,reason:'无法确认任务执行权限，请等待 Host 恢复'}; }
  });
