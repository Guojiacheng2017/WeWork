...
export function MarkdownMessage({ children, inverted = false }: { children: string; inverted?: boolean }) {
  const openLink = (event: React.MouseEvent<HTMLAnchorElement>, href?: string) => {
    if (!href || !/^https?:\/\//i.test(href)) return;
    event.preventDefault();
    const behavior = window.localStorage.getItem('wework.browser.linkBehavior') ?? 'system';
    if (behavior === 'copy') {
      void navigator.clipboard.writeText(href);
      window.dispatchEvent(new CustomEvent('wework:link-copied', { detail: href }));
      return;
    }
    if (window.weworkHost?.openExternal) void window.weworkHost.openExternal(href);
  ...
  return <div className="min-w-0 overflow-hidden break-words text-xs leading-6">
    <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
      ...
      a: ({ children, href }) => <a href={href} onClick={(event) => openLink(event, href)} className="cursor-pointer underline underline-offset-2">{children}</a>,
