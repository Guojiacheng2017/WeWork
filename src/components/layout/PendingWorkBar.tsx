...
export function PendingWorkBar({ embedded = false, onExpandedChange, onEmbeddedClose, onOpenTeamChat, auxiliaryDrawer }: { embedded?: boolean; onExpandedChange?: (expanded: boolean) => void; onEmbeddedClose?: () => void; onOpenTeamChat?: () => void; auxiliaryDrawer?: AuxiliaryDrawer }) {
  ...
  const submitChat=async(message = chatDraft)=>{if(!team||!message.trim()||chatSending)return;const parsed=parseGroupDraft(chatDraft,team.employees);if(!parsed.all&&parsed.mentioned.length>1){setChatError('请一次 @ 一位助手。');return false;}setChatSending(true);setChatError('');try{await sendTeamMessage(team.id,message.trim(),parsed.recipientId,parsed.contextTagIds);setChatDraft('');}catch(error){setChatError(error instanceof Error?error.message:String(error));return false;}finally{setChatSending(false);}};
