...
import { Activity, Archive, ArchiveRestore, ChevronRight, Cpu, LayoutDashboard, PanelLeftClose, PanelLeftOpen, Plus, Settings2, Trash2, UserRound, Users } from 'lucide-react';
  ...
  return (
    <aside aria-label="团队侧边栏" data-collapsed={collapsed} className={`${collapsed ? 'w-[64px]' : 'w-[240px]'} relative h-full bg-white border-r border-slate-200/90 flex flex-col select-none shrink-0 z-10 shadow-xs transition-[width] duration-200 ease-out`}>
      ...
      <div className={`flex-1 overflow-y-auto py-1 space-y-1 ${collapsed ? 'px-2' : 'px-3'}`}>
        <Button type="button" onClick={() => onPortalNavigate(null)} title={collapsed ? '协同工作台' : undefined} className={`flex w-full items-center rounded-xl border border-sky-100 bg-sky-50/70 text-xs font-semibold text-sky-800 transition-colors hover:border-sky-200 hover:bg-sky-100/70 ${collapsed ? 'h-10 justify-center' : 'gap-2 px-2.5 py-2'}`}><LayoutDashboard className="h-4 w-4" />{!collapsed && <><span className="flex-1 text-left">协同工作台</span><span className="rounded bg-white/80 px-1.5 py-0.5 text-[10px] font-medium text-sky-600">总览</span></>}</Button>
        <div className={`${collapsed?'mx-2 my-2':'my-3'} h-px bg-slate-100`} />
      ...
      <details ref={accountRef} className={`group relative border-t border-slate-100 [&>summary::-webkit-details-marker]:hidden ${collapsed ? 'p-2' : 'p-3'}`}>
        ...
        <div className={`absolute bottom-[calc(100%+8px)] z-50 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl ${collapsed ? 'left-2 w-52' : 'left-3 right-3'}`}>
          <div className="px-2.5 pb-1 pt-1.5"><strong className="block text-xs text-slate-700">本地用户</strong><span className="text-[11px] text-slate-400">WeWork · 本地模式</span></div>
          ...
          <div className="mx-2 my-1 h-px bg-slate-100" />
          <div className="flex items-center gap-2 px-2.5 py-2 text-[11px] text-slate-400"><span className="h-2 w-2 rounded-full bg-emerald-500" />本地模式</div>
        </div>
      </details>
    </aside>
  );
