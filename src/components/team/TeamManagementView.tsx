...
export function EmployeeConfigDialog({ employee, team, onClose, embedded = false }: { employee: WeWorkEmployee; team?: WeWorkTeam; onClose: () => void; embedded?: boolean }) {
  ...
  const [sessionExecution, setSessionExecution] = useState<SessionExecution>(() => initialExecution ?? {
    id: `session-${employee.id}`, name: `${employee.displayName} Session`, adapter: 'pi',
  ...
  useEffect(() => {
    void Promise.all([weworkHost.harnesses(), weworkHost.harnessPolicy(), weworkHost.harnessModels()]).then(([items, policy, catalog]) => {
      setAvailableHarnesses(items.filter((item) => (item.harness !== 'smalldashharness') && item.executionReady && policy.allowedHarnesses.includes(item.harness)));
      const availableModels = catalog.models.filter(model => model.harness !== 'smalldashharness');
...
export function TeamSettingsView({ team }: { team: WeWorkTeam }) {
...
export function TeamManagementView({ portalPage, onPortalNavigate, initialSection = 'members', embedded = false }: { portalPage: PortalPage | null; onPortalNavigate: (page: PortalPage | null) => void; initialSection?: 'members' | 'chat' | 'performance' | 'service'; embedded?: boolean }) {
