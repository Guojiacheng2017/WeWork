...
export function GroupMentionList({ employees, suggestions, selectedIndex, onSelect, className = '' }: {
  ...
  return <div role="listbox" aria-label="提及助手" className={`max-h-48 shrink-0 overflow-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg ${className}`}>
    {suggestions.map((suggestion, index) => {
      ...
      return <button key={suggestion.id} type="button" role="option" aria-selected={index === selectedIndex} onMouseDown={(event) => event.preventDefault()} onClick={() => onSelect(suggestion)} className={`ww-employee-control flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs text-sky-700 ${index === selectedIndex ? 'bg-sky-50' : ''}`}>
