...
export function ExecutionSettingsDialog({ onClose, initialSection = 'general' }: { onClose: () => void; initialSection?: SettingsSection | 'workspace' }) {
  ...
  const [modelDraft, setModelDraft] = useState({ name: '', provider: 'openai', modelId: '', baseUrl: '', credentialRef: '' });
  ...
  const saveAvailableModel = async () => {
    const input: HarnessModelInput = { harness, ...modelDraft, api: 'openai-completions', credentialRef: modelDraft.credentialRef || undefined, verified: false };
    setSaving(true);
    try {
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <section aria-labelledby="local-harnesses">
          ...
          <div className="grid grid-cols-2 gap-2">{installations.filter((item) => item.available).map((item) => {
            ...
            const canAllow = Boolean(item.executionReady && desktopHostAvailable);
        ...
        <HarnessModelCatalog harness={harness} setHarness={setHarness} installations={configurableInstallations} models={models} credentials={credentials} draft={modelDraft} setDraft={setModelDraft} saving={saving} save={() => void saveAvailableModel()} chooseDefault={(model) => void chooseDefaultModel(model)} />
