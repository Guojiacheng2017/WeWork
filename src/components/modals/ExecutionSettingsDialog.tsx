      ...
      <div className="flex min-h-0 flex-1">
        ...
        <section aria-labelledby="local-harnesses">
          ...
          <div className="grid grid-cols-2 gap-2">{displayedInstallations.map((item) => {
            ...
            return <article key={item.id} className={`rounded-xl border p-3 ${allowed && canAllow ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200'}`}><div className="flex items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center">{iconSource ? <img src={iconSource} alt="" className="h-6 w-6 object-contain" /> : <Cpu className="h-6 w-6 text-slate-500" />}</span><strong className="flex-1 text-xs text-slate-800">{harnessNames[item.harness]}{isSdh ? '（远程）' : ''}</strong><span className={`text-[10px] font-semibold ${item.executionReady ? 'text-emerald-600' : 'text-slate-400'}`}>{item.executionReady ? '● 已连接' : isSdh ? '未连接' : '仅检测到安装'}</span></div>{isSdh ? <><p className="mt-2 text-[10px] text-slate-400">SDH Docker 服务地址（不是模型 Base URL）</p><div className="mt-2 flex gap-2"><input aria-label="SDH 服务地址" value={sdhConnection.baseUrl} onChange={(event) => setSdhConnection({ baseUrl: event.target.value, configured: Boolean(event.target.value.trim()) })} placeholder="http://远程机器:23334" className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[10px]" /><button type="button" disabled={saving || !sdhConnection.baseUrl.trim()} onClick={async()=>{setSaving(true);try{const saved=await weworkHost.setSdhConnection(sdhConnection.baseUrl);setSdhConnection(saved);setMessage('远程 SDH 已连接');await refreshHarnesses();}catch(error){setMessage(error instanceof Error?error.message:String(error));}finally{setSaving(false);}}} className="shrink-0 rounded-lg bg-slate-900 px-3 text-[10px] font-semibold text-white disabled:opacity-40">连接测试</button></div>{sdhConnection.error && <p className="mt-1 text-[9px] text-rose-500">{sdhConnection.error}</p>}</> : <><p className="mt-2 truncate text-[10px] text-slate-400">{item.kind === 'embedded' ? 'WeWork 内置' : item.executablePath ?? installHelp[item.harness]}</p>{item.version && <p className="mt-1 text-[9px] text-slate-400">{item.version}{item.reason ? ` · ${item.reason}` : ''}</p>}</>}<div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3"><span className="text-[10px] font-medium text-slate-500">{allowed && canAllow ? '助手配置中可选' : '允许助手选择'}</span><button type="button" role="switch" aria-label={`允许助手使用 ${harnessNames[item.harness]}`} aria-checked={allowed && canAllow} disabled={!canAllow} onClick={() => void toggleAllowed(item)} className={`relative h-5 w-9 rounded-full transition-colors ${allowed && canAllow ? 'bg-emerald-500' : 'bg-slate-200'} disabled:cursor-not-allowed disabled:opacity-60`}><span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${allowed && canAllow ? 'left-[18px]' : 'left-0.5'}`} /></button></div>{!item.executionReady && !isSdh && <p className="mt-2 text-[9px] leading-4 text-slate-400">只发现了命令，尚未完成 WeWork 执行适配，因此不能分配给助手。</p>}</article>;
          })}</div>
        </section>
        <HarnessModelCatalog harness={harness} setHarness={setHarness} installations={configurableInstallations.filter((item) => item.harness === 'smalldashharness')} models={models} draft={modelDraft} setDraft={setModelDraft} saving={saving} save={() => void saveAvailableModel()} chooseDefault={(model) => void chooseDefaultModel(model)} status={message} />
