...
import { projectPluginName, projectPluginViews, usePluginCatalog } from './projectNavigation';
...
export function ProjectPluginLayout({ team, view, children }: { team: WeWorkTeam; view: TeamView; children: ReactNode }) {
  ...
  return <div className="flex h-full min-h-0 min-w-0">
    <nav aria-label={`${projectPluginName(team, catalog)} 导航`} className="w-40 shrink-0 overflow-y-auto border-r border-slate-200/70 bg-white/60 p-3 lg:w-48">
      <h2 className="px-3 pb-4 pt-3 text-sm font-semibold text-slate-900">{projectPluginName(team, catalog)}</h2>
