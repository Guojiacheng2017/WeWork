...
export class MemoryCollaborationRepository implements CollaborationRepository {
  ...
  async listIssues(teamId: string) { const team = await this.getTeam(teamId); if (!team) return []; return [...team.pendingWorks, ...team.employees.flatMap((employee) => [employee.currentWorkItem, ...(employee.queuedWorkItems ?? []), ...(employee.completedWorkItems ?? [])].filter((work): work is WorkItem => Boolean(work)))]; }
